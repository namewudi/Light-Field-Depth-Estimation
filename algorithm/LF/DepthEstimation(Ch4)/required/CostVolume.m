...
function E1 = CostVolume(LF,Sc,param,datatype,stop_temp,delta)
...
for n = 1 : numTarget
    ...
    for ell = 1 : numLabel
        ...
        alpha1 = stop_temp+delta-ell*delta;
            % Synthetic Dataset
        deltar =  Vn(1)*alpha1;
        deltac =  Vn(2)*alpha1;
        
        delta_f = [deltar deltac];
        In = fn_SubpixelShift(Ftarget, delta_f, row, col,1);
        SAD1 = min(sqrt( (In(:,:,1) - Ic(:,:,1)).^2 + (In(:,:,2) - Ic(:,:,2)).^2 + (In(:,:,3) - Ic(:,:,3)).^2 ),tau1);
        DSI_SAD1 = fn_PatchSum(SAD1,WindowSize);
               
        % Sum of the Gradient Difference
        
        Gx_n = fn_SubpixelShift(Ftarget_x, delta_f, row, col,0);
        Gy_n = fn_SubpixelShift(Ftarget_y, delta_f, row, col,0);
        ...
        E1(:,:,ell) = E1(:,:,ell)+(1-alpha).*DSI_SAD1 + alpha.*DSI_GRAD1;        
    end
    
    fprintf('CostVolume... %d / %d\n', n,numTarget)
