        ...
        for (x = 0; x < width; x++)
        {
            ...
            for (i = 0; i < angular_size; i++)
            {
                for (j = 0; j < angular_size; j++)
                {
                   ...
                   // if (abs(center_patch[(i*angular_size+j)*3+0]-cR)>abs(center_patch[(i*angular_size+j)*3+1]-cG))
                 //   {
                 //       dist=abs(center_patch[(i*angular_size+j)*3+0]-cR);
                 //   }
                //    else
                 //   {
                  //     dist= center_patch[(i*angular_size+j)*3+1]-cG;
                 //   }
                  //  if(dist<abs(center_patch[(i*angular_size+j)*3+2]-cB))
                   // {
                   //     dist=abs(center_patch[(i*angular_size+j)*3+2]-cB);
                 //   }
                    //dist=max(abs(center_patch[(i*angular_size+j)*3+0]-cR),abs(center_patch[(i*angular_size+j)*3+1]-cG));
                    //dist=max(round(dist) ,double(abs(center_patch[(i*angular_size+j)*3+2]-cB)));
                    pixel = center_patch[(i*angular_size+j)*3+0];
                    index = round(pixel);
                    histR1[index] = histR1[index] + exp(-1.0 * (pixel-cR)*(pixel-cR) / (2*sigma*sigma));
                    //histR1[index] = histR1[index] + exp(-1.0 * (dist)*(dist) / (2*sigma*sigma));
                    
                    pixel = center_patch[(i*angular_size+j)*3+1];
                    index = round(pixel);
                    histG1[index] = histG1[index] + exp(-1.0 * (pixel-cG)*(pixel-cG) / (2*sigma*sigma));
                    //histG1[index] = histG1[index] + exp(-1.0 * (dist)*(dist) / (2*sigma*sigma));
                    
                    pixel = center_patch[(i*angular_size+j)*3+2];
                    index = round(pixel);
                    histB1[index] = histB1[index] + exp(-1.0 * (pixel-cB)*(pixel-cB) / (2*sigma*sigma));
                    //histB1[index] = histB1[index] + exp(-1.0 * (dist)*(dist) / (2*sigma*sigma));
                }
            }
            sumR = 0;
            sumG = 0;
